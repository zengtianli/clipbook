...
/// 轮询 NSPasteboard.general.changeCount（macOS 没有剪贴板变更通知，Maccy/Deck/PastePal 都是轮询）。
///
/// 抓取优先级 **文件 > 文本（富文本随带）> 图片**：
///   · Finder 复制文件时同时带文件名文本 → 要文件不要文本；
///   · Word/Excel 复制一段字时同时带 TIFF 渲染图 → 要文本不要图；
///   · 只有纯图片（截图/浏览器拷图）才落图片。
/// 跳过：密码管理器标记的 ConcealedType / TransientType；忽略名单里的 app；自己刚写进去的那一次。
final class PasteboardWatcher {
    ...
    private func tick() {
        ...
        guard !ignoredBundles.contains(bundle) else { return }
        guard let cap = Self.extract(from: pb, appName: front?.localizedName ?? "", appBundle: bundle, plainTextOnly: plainTextOnly) else { return }
        onCapture(cap)
    }

    /// 纯函数：从任意 pasteboard 抽一条 Capture。SelfTest 用私有 pasteboard 直接驱动**这同一个函数**。
    static func extract(from pb: NSPasteboard, appName: String, appBundle: String, plainTextOnly: Bool = false) -> Capture? {
        ...
        if let s = pb.string(forType: .string), !s.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            if !plainTextOnly, let rtf = pb.data(forType: .rtf), !rtf.isEmpty, Classifier.kind(of: s) == .text {
                return Capture(kind: .richText, text: s, rtf: rtf, appName: appName, appBundle: appBundle)
            }
            return Capture(kind: Classifier.kind(of: s), text: s, appName: appName, appBundle: appBundle)
        }
        // ③ 图片
